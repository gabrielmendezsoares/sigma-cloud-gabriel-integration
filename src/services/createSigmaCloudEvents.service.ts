...
const getAlarmCode = (channelAlarm: IChannelAlarmMap.IChannelAlarmMap): string | null => {
  if (channelAlarm.motion_alarm && Object.hasOwn(CODE_MAP, "motion_alarm")) {
    return CODE_MAP['motion_alarm' as keyof typeof CODE_MAP];
  } else if (channelAlarm.motion_alarm !== undefined) {
  ...
  const intSubtype = channelAlarm.int_alarm?.int_subtype;
...
const processChannelAlarmList = async (
  ...
  const eventPayloadList: IEventPayloadMap.IEventPayloadMap[] = [];
        ...
        eventPayloadList.push(
          {
            ...
            complement: channelAlarmMap.chn_alias,
            eventId: EVENT_ID,
            eventLog: `Fabricante: Gabriel | Company ID: ${ deviceNamePartList[0] } | IP: ${ networkMap.ip } | MAC: ${ networkMap.mac }`,
...
export const createSigmaCloudEvents = async (
  ...
  try {
    ...
    const deviceNamePartList = body.data.device_name.split('_');

    await Promise.allSettled(alarmList.map((alarmMap: IAlarmMap.IAlarmMap): Promise<void> => processChannelAlarmList(alarmMap, body.data.dev_net_info[0], deviceNamePartList)));
